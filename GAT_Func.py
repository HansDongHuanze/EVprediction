...
class GAT_Fourier(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads, adj, num_nodes = 247, hidden_size_factor=1, embed_size=32, sparsity_threshold=0.01, levels=3):
        ...
        self.attentions = nn.ModuleList([
            FreqGraphAttentionLayer(nfeat // 2 + 1, nfeat // 2 + 1, dropout, alpha) 
            # GraphAttentionLayer(nfeat, nhid, dropout, alpha) 
            for _ in range(nheads)
        ])
        for i, attention in enumerate(self.attentions):
            self.add_module('attention_{}'.format(i), attention)
        self.norm = nn.LayerNorm(nfeat // 2 + 1)
        # self.out_att = GraphAttentionLayer(nhid * nheads, nclass, dropout=dropout, alpha=alpha, concat=False)
        self.encoder = nn.Linear(2, 1)
        self.activate = nn.LeakyReLU(0.01)
        self.decoder = nn.Linear(nfeat, 1)
        ...
        self.att_map = nn.Linear(num_nodes, self.nfeat // 2 + 1)

        self.gate_fusion = GateFusion(in_dim=nhid)

        self.W_q = nn.ParameterList([
            nn.Parameter(torch.randn(self.nfeat // 2 + 1, self.nfeat // 2 + 1)),
            nn.Parameter(torch.randn(self.nfeat // 2 + 1, self.nfeat // 2 + 1))
        ])
        self.W_k = nn.ParameterList([
            nn.Parameter(torch.randn(self.nfeat // 2 + 1, self.nfeat // 2 + 1)),
    ...
    def atten_com(self, x):
        ...
        for i in range(heads_stack.size(2)):
            ...
            fused_node = self.gate_fusion(node_features.reshape(self.nheads,-1,self.nfeat // 2 + 1))  # [512,7]
    ...
    def FGCN(self, x):
        ...
        # ifft
        x = torch.fft.irfft(x, n=self.nfeat, dim=2, norm="ortho")
        return x + res
